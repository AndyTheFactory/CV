...
public class Lab5Helper {
    ...
    {
        try{
            ImageIO.write(img,"jpg", new File(Lab5Helper.getNewFileName(fileName,suffix)));
        }catch(IOException E){
            System.out.println("Hauuuuu! nu pot scrie "+Lab5Helper.getNewFileName(fileName,suffix));
        }
    }
    public static void writeImageFile(BufferedImage img,File file,String suffix)
    {
        try{
            ImageIO.write(img,"jpg", new File(Lab5Helper.getNewFileName(file,suffix)));
        }catch(IOException E){
            System.out.println("Hauuuuu! nu pot scrie "+Lab5Helper.getNewFileName(file,suffix));
        }
    }
